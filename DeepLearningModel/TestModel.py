...
from DeepLearningModel.evaluate import *
import os
...
class TestModel(object):
    ...
    def test_AQNet_cnnlstm_lstm_embedding_normalvalue(self, X, Y, timestamps, len_history, meo_size, nb_meo_lstm_encode,
        ...
        for parent, dirnames, filenames in os.walk(path):
            for filename in filenames:
                ...
                if epoch_index > self.nb_start_epoch:
                    ...
                    all_normal_rmse.append(normal_rmse)
        ...
        for i in range(len(all_epoch)):
            info[all_epoch[i]] = str(all_normal_rmse[i]) + "," + str(all_normal_mse[i]) + "," + str(
                all_real_rmse[i]) + "," + str(all_real_smape[i])
        info_list = sorted(info.items(), key=lambda x: x[0], reverse=False)
        print("epoch, normal_rmse, normal_mse, real_rmse, real_smape")
        ...
        for parent, dirnames, filenames in os.walk(path):
            for filename in filenames:
                ...
                if epoch_index == fixed_epoch:
                    ...
                    Y_predict_real = inverse_transform_Y(Y_predic, self.Y_aq_max, self.Y_aq_min)
                    real_mse, real_rmse = computeRMSE(Y_predict_real, Y_real)
                    official_smape = smape(Y_real, Y_predict_real)
                    real_smape = computeSMAPE(Y_predict_real, Y_real)
                    all_normal_mse.append(normal_mse)
                    all_real_rmse.append(real_rmse)
                    all_real_smape.append(real_smape)
                    all_official_smape.append(official_smape)
                    ...
                    # PM2.5
                    saveResults(os.path.join(result_path, 'PM2.5_{}_nb{}.txt'.format(fixed_epoch, Y_predic.shape[0])),
                                Y_predict_real[:, :, 0], Y_real[:, :, 0], X[1][:, :, 0])
                    # PM10
                    saveResults(os.path.join(result_path, 'PM10_{}_nb{}.txt'.format(fixed_epoch, Y_predic.shape[0])),
                                Y_predict_real[:, :, 1], Y_real[:, :, 1], X[1][:, :, 1])
                    # O3
                    saveResults(os.path.join(result_path, 'O3{}_nb{}.txt'.format(fixed_epoch, Y_predic.shape[0])),
                                Y_predict_real[:, :, 2], Y_real[:, :, 2], X[1][:, :, 4])
        info = {}
        for i in range(len(all_epoch)):
            info[all_epoch[i]] = str(all_normal_mse[i]) + "," + str(all_real_rmse[i]) + "," + str(
                all_real_smape[i]) + "," + str(all_official_smape[i])
        info_list = sorted(info.items(), key=lambda x: x[0], reverse=False)
        print("epoch, normal_mse, real_rmse, real_smape, official_smape")
        for info_item in info_list:
            print(info_item)

    def test_AQNet_cnnlstm_lstm_embedding_normalvalue_saveResultsWithTime(self, meo_max, meo_min, aq_max, aq_min, X, Y,
                                                                          timestamps, len_history, meo_size,
                                                                          nb_meo_lstm_encode,
                                                                          nb_meo_lstm_decode, nb_aq_lstm_encode,
                                                                          nb_aq_lstm_decode,
                                                                          external_dim, nb_meo_conv=2, fixed_epoch=1):
        ...
        for parent, dirnames, filenames in os.walk(path):
            for filename in filenames:
                ...
                if epoch_index == fixed_epoch:
                    ...
                    print("X[1].shape:", X[1].shape)
                    # PM2.5
                    saveResultsWithTime(timestamps, os.path.join(result_path, 'PM2.5_{}_nb{}.txt'.format(fixed_epoch,
                                                                                                         Y_predic.shape[
                                                                                                             0])),
                                        Y_predict_real[:, :, 0], Y_real[:, :, 0], X[1][:, :, 0])
                    # PM10
                    saveResultsWithTime(timestamps, os.path.join(result_path, 'PM10_{}_nb{}.txt'.format(fixed_epoch,
                                                                                                        Y_predic.shape[
                                                                                                            0])),
                                        Y_predict_real[:, :, 1], Y_real[:, :, 1], X[1][:, :, 1])
                    # O3
                    saveResultsWithTime(timestamps, os.path.join(result_path, 'O3{}_nb{}.txt'.format(fixed_epoch,
                                                                                                     Y_predic.shape[
                                                                                                         0])),
                                        Y_predict_real[:, :, 2], Y_real[:, :, 2], X[1][:, :, 4])
        info = {}
