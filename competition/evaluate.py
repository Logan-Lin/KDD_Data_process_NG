...
def cal_smape(predict_dt, file_suffix=None, predict_csv=None, city="bj"):
    ...
    bj_true_data = load_data.load_directory_data(load_data.api_data_directory.format(city, "aq"),
    ...
    for aq_name in aq_names:
        true_span_df = bj_true_data.loc[aq_name, :].loc[predict_dt:predict_dt + td(hours=47), real_column]
    ...
    predict_str = predict_dt.strftime(format_string[1])
    column_smapes = []
    for i in range(len(predict_column)):
        ...
        print("{} {} {} - {}".format(city, predict_str, predict_column[i], smape_value))

    all_predict = np.array(concat_df.loc[:, predict_column])
    all_real = np.array(concat_df.loc[:, real_column])
    smape_value = smape(all_real, all_predict)
    column_smapes.append(smape_value)
    print("{} {} Aggregate - {}".format(city, predict_str, smape_value))
        ...
        for filename in filenames:
            ...
            if ext == ".csv":
                ...
                bj_smape = cal_smape(
                    ...
                    predict_csv=file_dir, file_suffix=name, city="bj")
                ld_smape = cal_smape(
                    predict_dt=dt.strptime((name.split("_"))[0], format_string[2]).strftime(format_string[1]),
                    predict_csv=file_dir, file_suffix=name, city="ld")
                table.append(bj_smape + ld_smape[1:])
                print()
    df = pd.DataFrame(table, columns=["file_name", "pm2.5_bj", "pm10_bj", "o3_bj",
